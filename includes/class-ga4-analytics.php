...
class GA4_Analytics {
    ...
    public function __construct() {
        ...
        $this->property_id = preg_replace('/^properties\//', '', (string) get_option('ga4_property_id', ''));
    ...
    private function make_api_request($endpoint, $params = []) {
        ...
        if (!empty($this->api_key)) {
            $url .= '?key=' . rawurlencode($this->api_key);
        }
        ...
        if (is_wp_error($response)) {
            error_log('GA4 Analytics: API request failed (' . $endpoint . '): ' . $response->get_error_message());
            return false;
        }
        ...
        if ($status_code < 200 || $status_code >= 300) {
            $message = is_array($decoded) && isset($decoded['error']['message'])
                ...
                : 'HTTP ' . $status_code;
            error_log('GA4 Analytics: API error (' . $endpoint . '): ' . $message);
            return false;
        }

        if (isset($decoded['error'])) {
            error_log('GA4 Analytics: API error (' . $endpoint . '): ' . ($decoded['error']['message'] ?? 'Unknown error'));
            return false;
        }
    ...
    /**
     * Empty page views payload used when a report request fails.
     */
    private function empty_page_views_data() {
        return [
            'labels' => [],
            'data' => [],
            'top_pages' => [],
    ...
    /**
     * Empty traffic sources payload used when a report request fails.
     */
    private function empty_traffic_sources_data() {
        return [
            'labels' => [],
            'data' => [],
            'sources' => [],
    ...
    public function get_page_views($start_date = '7daysAgo', $end_date = 'today') {
        // Use page-compatible metrics only. averageSessionDuration is session-scoped
        // and causes GA4 API errors when combined with pagePath.
        $request_body = [
            'dateRanges' => [
        ...
        if (isset($response['rows'])) {
            foreach ($response['rows'] as $row) {
                ...
                $engagement = isset($row['metricValues'][1]['value']) ? floatval($row['metricValues'][1]['value']) : 0;
                $avg_time = $views > 0 ? round($engagement / $views) : null;
    ...
    /**
     * Period configuration: date ranges and chart granularity
     */
    private function get_period_config($period) {
        $configs = array(
            'day' => array(
                'start' => 'today',
                'end' => 'today',
                'days' => 1,
                'trend_dimension' => 'hour',
                'trend_limit' => 24,
                'label' => 'Today',
            ),
            'week' => array(
    ...
    /**
     * Convert GA4 relative date strings to Y-m-d.
     */
    private function resolve_ga4_date($date) {
        ...
        if (preg_match('/^(\d+)daysAgo$/', (string) $date, $matches)) {
            return wp_date('Y-m-d', strtotime('-' . (int) $matches[1] . ' days'));
        }
    ...
    /**
     * Human-readable date range for the dashboard UI.
     */
    private function format_date_range_display($start_date, $end_date) {
        $start = $this->resolve_ga4_date($start_date);
        $end = $this->resolve_ga4_date($end_date);
    ...
    /**
     * Build trend chart config for a custom date span.
     */
    private function get_custom_period_config($start_date, $end_date) {
        $start_ts = strtotime($start_date);
        $end_ts = strtotime($end_date);
        $days = max(1, (int) round(($end_ts - $start_ts) / DAY_IN_SECONDS) + 1);
        ...
        if ($days === 1) {
            ...
            $trend_dimension = 'yearMonth';
            $trend_limit = min(24, (int) ceil($days / 30));
        } else {
            $trend_dimension = 'date';
            $trend_limit = min(90, $days);
        }
        ...
        return array(
            ...
            'label' => $this->format_date_range_display($start_date, $end_date),
        );
    }

    /**
     * Previous period range for comparison metrics
     */
    private function get_previous_period_range($period) {
        $config = $this->get_period_config($period);
        $days = $config['days'];
        ...
        return array(($days * 2) . 'daysAgo', ($days + 1) . 'daysAgo');
    }

    /**
     * Comparison period range, including custom date selections
     */
    private function get_comparison_period_range($period, $start_date, $end_date, $custom_start_date = null, $custom_end_date = null) {
        if ($period === 'custom' && $custom_start_date && $custom_end_date) {
            $start_ts = strtotime($custom_start_date);
            $end_ts = strtotime($custom_end_date);
            ...
            if ($start_ts && $end_ts && $end_ts >= $start_ts) {
                ...
                return array(
                    date('Y-m-d', $prev_start_ts),
                    date('Y-m-d', $prev_end_ts),
                );
            }
        }

        return $this->get_previous_period_range($period);
    }
    ...
    public function get_analytics_data($period = 'day', $custom_start_date = null, $custom_end_date = null) {
        if ($period === 'custom' && $custom_start_date && $custom_end_date) {
            ...
            $period_config = $this->get_custom_period_config($start_date, $end_date);
        } else {
            $period_config = $this->get_period_config($period);
            $start_date = $period_config['start'];
            $end_date = $period_config['end'];
        }

        $date_range_display = $this->format_date_range_display($start_date, $end_date);
        ...
        $daily_trend = $this->get_trend_chart_data($start_date, $end_date, $period_config);

        $data_warnings = array();
        if ($page_views_data === false) {
            error_log('GA4 Analytics: Failed to fetch top pages report');
            $page_views_data = $this->empty_page_views_data();
            $data_warnings[] = 'Top pages data could not be loaded from GA4.';
        }
        if ($traffic_sources_data === false) {
            error_log('GA4 Analytics: Failed to fetch traffic sources report');
            $traffic_sources_data = $this->empty_traffic_sources_data();
            $data_warnings[] = 'Traffic source data could not be loaded from GA4.';
        }
        if (empty($page_views_data['top_pages']) && empty($data_warnings)) {
            $data_warnings[] = 'No page data for this period yet. Try Week or Month — GA4 can take several hours to process today\'s reports.';
        }

        list($prev_start, $prev_end) = $this->get_comparison_period_range($period, $start_date, $end_date, $custom_start_date, $custom_end_date);
        $prev_total_views = $this->get_total_page_views($prev_start, $prev_end);
        ...
        $bounce_rate = null;
        if (is_array($basic_metrics) && isset($basic_metrics['bounce_rate'])) {
            $bounce_rate = round(floatval($basic_metrics['bounce_rate']) * 100, 1);
        }
        
        return [
            'connection_status' => 'connected',
            'period' => $period,
            'date_range_label' => $period_config['label'],
            'date_range_display' => $date_range_display,
            'date_range_start' => $this->resolve_ga4_date($start_date),
            'date_range_end' => $this->resolve_ga4_date($end_date),
            ...
            'sessions' => is_array($basic_metrics) && isset($basic_metrics['sessions']) ? $basic_metrics['sessions'] : 0,
            ...
            'trending_insights' => $this->build_dashboard_insights($page_views_data, $traffic_sources_data, $change_percent),
            'recent_activity' => $this->get_recent_activity()
        ];
    }

    /**
     * Build simple insights from live GA4 metrics
     */
    private function build_dashboard_insights($page_views_data, $traffic_sources_data, $change_percent) {
        $insights = array();

        if (!empty($page_views_data['top_pages'][0])) {
            $top_page = $page_views_data['top_pages'][0];
            $insights[] = array(
                'type' => 'page',
                'title' => 'Top Page',
                'description' => $top_page['page'] . ' leads with ' . number_format($top_page['views']) . ' views',
                'impact' => 'high',
            );
        }

        if (!empty($traffic_sources_data['sources'][0])) {
            $top_source = $traffic_sources_data['sources'][0];
            $insights[] = array(
                'type' => 'source',
                'title' => 'Top Traffic Source',
                'description' => $top_source['source'] . ' drives ' . $top_source['percentage'] . '% of sessions',
                'impact' => 'medium',
            );
        }
        ...
        if ($change_percent != 0) {
            $insights[] = array(
                'type' => 'trend',
                'title' => 'Traffic Trend',
                'description' => 'Page views are ' . ($change_percent >= 0 ? 'up' : 'down') . ' ' . abs($change_percent) . '% vs the previous period',
                'impact' => abs($change_percent) >= 10 ? 'high' : 'medium',
            );
        }
    ...
    /**
     * Get trend chart data with period-appropriate granularity
     */
    public function get_trend_chart_data($start_date, $end_date, $period_config) {
        $dimension = $period_config['trend_dimension'] ?? 'date';
        $limit = $period_config['trend_limit'] ?? 31;
        ...
        $request_body = array(
            ...
            'orderBys' => array(
                array('dimension' => array('dimensionName' => $dimension), 'desc' => false)
            ),
            'limit' => $limit
        );
        ...
        if (!$response || isset($response['error']) || empty($response['rows'])) {
            return array('labels' => array(), 'data' => array(), 'granularity' => $dimension);
        }
        ...
        foreach ($response['rows'] as $row) {
            $raw_value = $row['dimensionValues'][0]['value'];
            $labels[] = $this->format_trend_label($raw_value, $dimension);
            $data[] = intval($row['metricValues'][0]['value']);
        }
    ...
    /**
     * Format chart axis labels based on GA4 dimension type
     */
    private function format_trend_label($raw_value, $dimension) {
        switch ($dimension) {
            case 'hour':
                $hour = intval($raw_value);
                if ($hour === 0) {
                    return '12 AM';
                }
                if ($hour === 12) {
            ...
            case 'yearMonth':
                if (strlen($raw_value) >= 6) {
                    $month_num = intval(substr($raw_value, 4, 2));
                    $year = substr($raw_value, 0, 4);
                    $months = array('', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec');
                    return ($months[$month_num] ?? $month_num) . " '" . substr($year, 2, 2);
                }
                return $raw_value;

            case 'yearWeek':
                if (strlen($raw_value) >= 6) {
                    return 'W' . substr($raw_value, 4, 2);
                }
                return $raw_value;

            case 'date':
            default:
                if (strlen($raw_value) === 8) {
                    return intval(substr($raw_value, 4, 2)) . '/' . intval(substr($raw_value, 6, 2));
                }
                return $raw_value;
        }
    }
    ...
    /**
     * @deprecated Use get_trend_chart_data instead.
     */
    public function get_daily_page_views($start_date = '7daysAgo', $end_date = 'today') {
        return $this->get_trend_chart_data($start_date, $end_date, $this->get_period_config('week'));
    }
     ...
     */
    /**
     * @deprecated Use get_previous_period_range instead.
     */
    private function get_previous_period_start($start_date, $period) {
        $range = $this->get_previous_period_range($period);
        return $range[0];
    }
